...
public class EditProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (new PermissionManager(EditProfileActivity.this).checkPreference()) {
            btnAllowPermissions.setVisibility(View.GONE);
        }
        Toolbar toolbar = findViewById(R.id.edit_profile_toolbar);
        setSupportActionBar(toolbar);
        getSupportActionBar().setTitle(getResources().getString(R.string.edit_infos));
        ...
        username.getEditText().setText(userUsernameSharedPrefs.getString("userUsernameSharedPrefs", ""));
        paypal = (TextInputLayout) findViewById(R.id.paypal_url_edit_text);
        if (userPaypalSharedPrefs.getString("userPaypalSharedPrefs", "").equals("null")) {
            paypal.getEditText().setText("");
        } else {
            paypal.getEditText().setText(userPaypalSharedPrefs.getString("userPaypalSharedPrefs", ""));
        }
        profilePic = (CircleImageView) findViewById(R.id.profile_image);
        Picasso.get().load(userAvatarUrlSharedPrefs.getString("userAvatarUrlSharedPrefs", "")).fit().centerInside().into(profilePic);
        save = (Button) findViewById(R.id.save_button);
        editSocial = (Button) findViewById(R.id.edit_social_btn);
        editSocial.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent social = new Intent(EditProfileActivity.this, EditProfileSocialActivity.class);
                startActivity(social);
            }
        });
        vegetarian = (Spinner) findViewById(R.id.spinner_vegetarian);
        if (userVegetarianSharedPrefs.getString("userVegetarianSharedPrefs", "").equals("yes")) {
            vegetarian.setSelection(1);
        } else {
            vegetarian.setSelection(0);
        }
                ...
                if (validator.validate()) {
                    if (Objects.requireNonNull(paypal.getEditText()).getText().toString().isEmpty()) {
                        ...
                        if(paypal.getEditText().getText().toString().contains("paypal.me")) {
                            ...
                            Toast.makeText(EditProfileActivity.this, getResources().getString(R.string.paypal_error), Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            }
        ...
        switch (requestCode) {
            case REQUEST_WRITE_PERMISSION : {
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED && grantResults[1] == PackageManager.PERMISSION_GRANTED   ) {
                    new PermissionManager(EditProfileActivity.this).writePreference();
                    // Hide Image
                    btnAllowPermissions.setVisibility(View.GONE);
                }
                return;
    ...
    private void uploadImage() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url+"/api/chefs/image/upload", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                Toast.makeText(EditProfileActivity.this, getResources().getString(R.string.image_changed), Toast.LENGTH_SHORT).show();
            }
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url+"/api/chef/complete", new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if (success.equals("1")) {
                        Toast.makeText(EditProfileActivity.this, getResources().getString(R.string.infos_saved), Toast.LENGTH_SHORT).show();
                        userUsernameSharedPrefs.edit().putString("userUsernameSharedPrefs", usernameStr).apply();
                        userPaypalSharedPrefs.edit().putString("userPaypalSharedPrefs", paypalStr).apply();
                        userVegetarianSharedPrefs.edit().putString("userVegetarianSharedPrefs", vegetarianStr).apply();
                        Intent social = new Intent(EditProfileActivity.this, MyProfileActivity.class);
